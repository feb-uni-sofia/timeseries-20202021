...
$$
y_t = \alpha t + u_{t} \quad M1: \text{ linear trend} \\
y_t = \alpha + y_{t - 1}  +u_{t} = \alpha t + \sum_{i = 1}^{t}u_{i} \quad M2: \text{ random walk with drift}
$$

$$
M1: E(y_t) = \alpha t\\
M2: y_{t} = \alpha t + \sum_{i = 1}^{t}u_{i}
$$
...
\Delta y_{t} = y_{t} - y_{t - 1} = \alpha + u_{t}
$$
...
# plot(timeSeries)
getwd()
## save(fit.xy, file = "./test_model.fit.RData")
load(file = "./test_model.fit.RData")
```
